...
func swapHandler(deps ChainDeps, work SwapWork, log zerolog.Logger) func(context.Context, *asynq.Task) error {
	return func(ctx context.Context, t *asynq.Task) error {
		...
		// Routing tiles are NOT per-region anymore. The first region
		// install's pipeline:routing stage already enqueued a one-time
		// download of the world routing tar; subsequent regions just
		// observe the tar is in place and skip.
		return nil
	}
}
